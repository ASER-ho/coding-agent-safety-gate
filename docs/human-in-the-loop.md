# 用户参与的工作流 / Human-In-The-Loop Workflow
...
一个 Agent 可以在执行边界任务方面非常出色，但当它默默扩大使命时就变得不安全。Human-in-the-loop 工作流将权力保留在用户手中，让 Agent 负责窄范围执行加证据。
...
- 识别用户审查的决策点 / identifies decision points for human review
...
1. 用户编写或批准项目规则。 / Human writes or approves project rules.
2. 用户或规划助手起草下一个任务。 / Human or planning assistant drafts the next task.
3. 用户批准任务范围和禁止操作。 / Human approves the task scope and forbidden operations.
...
6. 用户审查验收清单。 / Human reviews the acceptance checklist.
7. 用户决定是否由另一个 Agent 继续。 / Human decides whether another agent should continue.
8. 在下一个 Agent 开始前更新 handoff 记录。 / Handoff notes are updated before the next agent starts.

## 必须由用户决策的节点 / Required Human Decision Points

在 AI Agent 执行以下操作前，用户必须确认 / A human must confirm before an AI agent:
...
- 下一步是否需要用户批准 / whether human approval is required for the next step
