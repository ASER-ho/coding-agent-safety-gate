# Claude Code — 安全门示例 / Safety Gate Example

## Purpose / 用途

本示例展示如何在 Claude Code（或其他终端型编程助手）中使用 `coding-agent-safety-gate`。它是一层文档优先的工作流——不是 runtime enforcement，不是插件，不是包。

This example shows how to use `coding-agent-safety-gate` with Claude Code (or another terminal-based coding agent). It is a documentation-first workflow layer — not runtime enforcement, not a plugin, not a package.

## 推荐给 Agent 的文件 / Recommended files to give the agent

在开始 Claude Code 会话前，从 `templates/` 复制这些文件到你的项目中并填写：

Copy these files from `templates/` into your project and fill them in before starting a Claude Code session:
...
- `HANDOFF.md` — 下一个人类或 Agent 的上下文 / context for the next human or agent
...
1. **人类** 编写或审查 `PROJECT_RULES.md`。 / **Human** writes or reviews `PROJECT_RULES.md`.
2. **人类** 编写恰好一个 `NEXT_TASK.md`。 / **Human** writes exactly one `NEXT_TASK.md`.
...
6. **人类** 使用 `GIT_DIFF_REVIEW_CHECKLIST.md` 审查 diff。 / **Human** reviews the diff using `GIT_DIFF_REVIEW_CHECKLIST.md`.
7. **人类** 在分享或发布前使用 `SAFETY_REDACTION_CHECKLIST.md` 检查输出。 / **Human** checks the output using `SAFETY_REDACTION_CHECKLIST.md` before sharing or publishing.
8. **人类** 决定是否 commit、push、tag 或 release。 / **Human** decides whether to commit, push, tag, or release.

Agent 不批准自己的工作，也不决定下一个任务。将人类批准保持在 Agent 之外。

The agent does not approve its own work or decide the next task. Keep human approval outside the agent.

## 禁止的 Agent 行为 / Forbidden agent behavior

使用 Claude Code 或其他终端型编程 Agent 时，Agent 不得 / When using Claude Code or another terminal-based coding agent, the agent must not:
...
- 未经人类批准继续进入下一个任务 / continue into the next task without human approval
- 没有证据声称完成 / claim completion without evidence

## 示例提示词 / Example prompt

在设置好项目文件后，将此复制到 Claude Code 会话中。 / Copy this into a Claude Code session after setting up your project files:
...
Before editing, read .ai/PROJECT_RULES.md and .ai/NEXT_TASK.md.
...
人类审查者拥有所有最终决定权。Agent 的执行报告是支持审查的证据——不是声称工作正确、安全或可以发布的证明。在 commit、push、tag 或 release 之前审查每个变更。
